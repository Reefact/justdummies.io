...
# The consent module is served from public/ rather than bundled, because the
# playground's hand-written shell loads it at a fixed address and cannot learn a
# fingerprinted one (ADR-0025). Unfingerprinted means a changed file is the same URL,
# so it is revalidated like a document: a cached copy of this one would be a stale
# decision — a withdrawal path fixed and not delivered, or a retention window that
# went on being the old one.
/consent.js
  Cache-Control: public, max-age=0, must-revalidate
