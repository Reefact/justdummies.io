#!/bin/sh
# Validate a commit message against the justdummies.io commit convention.
#
...
#
# The convention is the library's, adopted deliberately: justdummies.io and
# JustDummies are two repositories but one product, and a maintainer moving
# between them should not have to remember which set of rules is in force. Only
...
# the local hook lets them through. CI (--ci) rejects them instead: this
# repository lands pull requests by rebase, so a placeholder merged before its
...
if [ -z "$subject" ]; then
  ...
  if printf '%s' "$subject" | grep -Eq "^(${TYPES})(\((${SCOPES})(,(${SCOPES}))*\))?!?: [a-z]"; then
    ...
    # a capability added or a defect fixed here always belongs to one of the
    # components, and the diff does not always say which -- a change under
    # packages/design-tokens serves both applications. See CONTRIBUTING.md ->
    # "Scope".
    case "$subject" in
      feat:*|feat!:*|fix:*|fix!:*)
        vtype="${subject%%[:!(]*}"
        err "a '${vtype}' commit must carry a scope, e.g. '${vtype}(site): …' — a capability or a defect always belongs to one component, and the diff alone does not always name it"
        ;;
    ...
    if printf '%s' "$subject" | grep -Eq '^[a-z]+\([^)]*, '; then
      err "scopes are comma-separated with no space: '(playground,site)', not '(playground, site)'"
    fi
    ...
    if printf '%s' "$subject" | grep -Eq '^[a-zA-Z]+\('; then
      ...
      for s in $grp; do
        ...
        if ! printf '%s' "$st" | grep -Eq "^(${SCOPES})$"; then
          err "unknown scope '${st}' — use one of: ${SCOPES_HUMAN} (a scope names a component, never a file or a directory)"
        fi
      done
      IFS=$OLDIFS
    fi
